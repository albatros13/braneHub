...
# I modified this to be acceptyed by syntax
  some_listed = {count(acceptable) > 0}
  some_listed = {count(conditional) > 0}
...
# Error on this line
  has_acc = false("Storage '%v': provide one of acceptable (%v) or conditional (%v) formats.",
  [kind, concat_arr(acceptable), concat_arr(conditional)])]
} else = [] { true }
